...
> 状态：已按本文核心结构落地（2026-08-27）
>
> 代码基线：2026-08-26 当前工作区
>
...
工具名称、description 和 input schema 通过 API `tools` 发送。System Prompt 只描述如何
...
| 通道 | 当前内容 |
| --- | --- |
| system | base、Memory 行为说明、style、MCP 工具清单、Skill 摘要、voice、cache breaker |
| context user message | 日期、Memory、workspace、MCP server/instructions、会话摘要 |
| tools | 本地工具和 MCP 工具的名称、description、参数 Schema |
...
| 位置 | 现在在干什么 |
| --- | --- |
| `backend/prompts/prompting.py` | 拼 system，读 Memory，抽路径 |
| `backend/runners/k_agent.py::create_runtime` | 再往 `user_context` 里塞 workspace / MCP / instructions |
| `backend/memory/renderer.py`、`voice_prompt.py`、`mcp_prompt.py` | 各自产出一段字符串 |

结果是：不知道最终发给模型的内容从哪来、谁能覆盖谁、哪段该进 system、哪段该进工具 Schema。`create_runtime` 也因此既连 MCP、又绑工具、又手改 Prompt。

本文只定两件事：

1. **K Agent 的 system/context 注入文本，都由 `backend/prompts/` 产出。**
2. **拼接只有一个函数。`create_runtime` 组好输入后调用它，不再自己拼字符串。**

### 2.3 当前实现的具体问题

- MCP tool description 同时进入 system 和 Provider `tools`，重复且容易失配。
- Skill 目录进入 system，而不是由请求级 `Skill` 工具拥有。
- 所有 Memory 共用“OVERRIDE any default behavior”，没有区分 Managed、项目指令、自动记忆
  和外部 MCP 信息的权限。
- `user_context: dict[str, str]` 无法表达来源、权限、生命周期和敏感性。
- `build_nested_memory_context()` 已存在，但未进入真实工具 Observation 循环。
- Access Layer 和 Agent Backend 各有一份 `DEFAULT_SYSTEM_PROMPT`，但实际编译权在 Agent Backend。
...
- Skill 目录写进 `Skill` 工具的 description；Skill 正文仍走 tool result。
...
它不是普通 Section Builder。Skill catalog 和工具 description 归 `backend/tools/`，不属于
Prompt 子目录。
...
- Skill 目录和动态 Tool description：由 `backend/tools/` 根据同一份 `SkillCatalog` 生成。
...
# 3. 连 MCP、校验模型，生成 SkillCatalog，再绑定本轮工具
...
`tool_guidance` 和 Skill 工具 description 必须描述真实暴露的工具；先拼 Prompt 再绑工具会说错能力。
...
Skill 不再形成“先绑定还是先 compose”的循环：`SkillCatalog` 先由选中的 Skill 构造，
`build_skill_tool()` 同时用它生成 description 和执行闭包；`compose_prompt()` 只读取最终
Catalog 中是否存在 `Skill`，不负责生成或修改工具 description。
...
system 只保留一句：匹配任务时调用 `Skill` 工具。

本轮选中的 Skill 先形成只读 `SkillCatalog`，再由 `backend/tools/local.py::build_skill_tool()`
同时生成 description 和绑定执行闭包：

```text
Load one selected skill when it matches the task.

Available skills:
- pdf: Read, create, edit, and verify PDF files.
```

完整正文仍只在调用 Skill 之后作为 tool result 出现。未选中的 Skill 对模型不可见。

`compose_prompt()` 不返回 `skill_tool_description`，只根据最终 `ToolCatalog` 判断是否需要
加入一句通用 Skill guidance。
...
| Skill 摘要追加进 system | 工具层用 `SkillCatalog` 生成请求级 `Skill` description |
...
- MCP 工具目录和 Schema 不重复进入 system/context 前缀；Skill 目录只在 Skill 工具
  description；CLAUDE.md 在 context；MANAGED 在 system。历史 tool call/result 或用户原话
  出现工具名不属于重复注入。
...
当前实现已完成下列迁移：typed `PromptSection/PromptBundle`、唯一 `compose_prompt()`、
`SkillCatalog/ToolCatalog`、`instruction_root/output_workspace` 分离、Memory 权限分层、MCP/Skill
...
每一步都保持现有 ReAct / HITL / 工具历史测试通过，不要一次改完所有字符串。
...
4. **去重**：删 MCP system 清单；Skill 目录改走工具 description。
